...
#include <algorithm>    // equal, lower_bound
...
Container fordJohnsonAlgorithm(Container c)
{
    ...
    {
        size_t jacobIndex = J[i];
        if (jacobIndex < losers.size())
        {
            typename Container::iterator pos = std::lower_bound(winers.begin(), winers.end(), losers[jacobIndex]);
            winers.insert(pos, losers[jacobIndex]);
        }
    }
...
void PmergeMe::displayResult()
{
    ...
    {
        std::cerr << "Container size has changed after the sorting proccess.\n";
        return;
    }
    ...
    if (deq.size() != vec.size() || !std::equal(deq.begin(), deq.end(), vec.begin()))
    {
        std::cerr << "Deque and vector container contents are not the same.\n";
        return;
    }
    ...
    std::cout << "Time to process a range of " << deq.size() << " elements with std::deque  : " << deq_time << " ms\n";
    std::cout << "Time to process a range of " << vec.size() << " elements with std::vector : " << vec_time << " ms\n";
}
